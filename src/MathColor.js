...
/**
 * MathColor class
 * Represet the MathColor object and it's different types (HEX, RGBA, XYZ, LAB)
 * This class have the ability to do the following
 * 1. Convert HEX to RGBA
 * 2. Convert RGB to XYZ
 * 3. Convert XYZ to LAB
 * 4. Calculate Delta E00 between two LAB MathColor (Main purpose)
 * @author Ahmed Moussa <moussa.ahmed95@gmail.com>
 * @version 1.5
 */
export default class MathColor {
  /**
   * Convert HEX to LAB
   * @param {[string]} hex hex MathColor value desired to be converted to LAB
   */
  static HEX2LAB(hex) {
    const [R, G, b, a] = MathColor.HEX2RGBA(hex);
    const [X, Y, Z] = MathColor.RGB2XYZ(R, G, b, a);
    const [L, A, B] = MathColor.XYZ2LAB(X, Y, Z);
    return [L, A, B];
  }
  ...
  static LAB2RGBA(L, A, B) {
    const [X, Y, Z] = MathColor.LAB2XYZ(L, A, B);
    const [R, G, b, a] = MathColor.XYZ2RGBA(X, Y, Z);
    return [R, G, b, a];
  }

  /**
   * Convert HEX to RGBA
   * @param {[string]} hex hex MathColor value desired to be converted to RGBA
   */
  static HEX2RGBA(hex) {
    let c;
    if (hex.charAt(0) === '#') {
    ...
    if (c.length > 6 || c.length < 3) {
      throw new Error(
        `HEX MathColor must be 3 or 6 values. You provided it ${c.length}`
      );
    }
    if (c.length === 3) {
  ...
  static XYZ2RGBA(X, Y, Z) {
    ...
    const R = Math.round(var_R * 255);
    const G = Math.round(var_G * 255);
    const B = Math.round(var_B * 255);
   ...
   * @param {[type]} l1 MathColor 1
   * @param {[type]} a1 MathColor 1
   * @param {[type]} b1 MathColor 1
   * @param {[type]} l2 MathColor 2
   * @param {[type]} a2 MathColor 2
   * @param {[type]} b2 MathColor 2
   */
  ...
  /**
   * Get darker MathColor of the given MathColor
   * @param {[Number]} R     Red value from 0 to 255
   * @param {[Number]} G     Green value from 0 to 255
   * @param {[Number]} B     Blue value from 0 to 255
   */
  static GetDarkerColour(r, g, b, a = 1, darkenPercentage = 0.05) {
    let [L1, A1, B1] = MathColor.RGBA2LAB(r, g, b, a);
    L1 -= L1 * darkenPercentage;
    if (L1 < 0) {
      L1 = 0;
    }
    const [R, G, B, A] = MathColor.LAB2RGBA(L1, A1, B1);
    return [R, G, B, A];
  }

  /**
   * Get brighter MathColor of the given MathColor
   * @param {[Number]} R     Red value from 0 to 255
   * @param {[Number]} G     Green value from 0 to 255
   * @param {[Number]} B     Blue value from 0 to 255
   */
